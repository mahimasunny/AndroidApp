...
#define WINDOWSIZE 100     // 10Hz * 10 seconds, so about 400 samples (could be less)
...
void updateThreshold () {
  ...
  if (Threshold < 520) { Threshold = 520; } // min threshold
//  if (dbg) {
...
int findNumBeatsPerMin() {
  ...
  // for 10 sec, numSamples will be approx 10*10 for 10Hz
  // for 5 sec, numSamples will be approx 10*5 and min number of beats will be 30bpm * 5/60, i.e. 30 * (numsamples/10) / 60 
  // e.g. for full 10 sec, min number of beats will be 30 * (100 / 10) / 60, or 10 beats as discussed above
  //                   and max number of beats will be 300 * (100 / 10) / 60 or 100 beats in 20 sec, or 300 bpm
...
//  }
  ...
  return numBeats * 6; // 20 sec data, so multiply by 3
...
//  }
  for (int i = 1; i < numSamples - 1; ++i) {
    ...
    if (curVal >= rawAccels[i-1] && curVal >= rawAccels[i+1]) {
      // is a peak
      if (i - lastPeak < 6) {
        continue;
      }
...
void loop() {
  ...
  if (panic == 1) {
      data = "critical:" + String(numBpm);
  } else if (now - lastSendTime >= 1000) {
      lastSendTime = now;
      // send ble data
      data = "bpm:" + String(numBpm);
  }
  if (data != "") {
      // Serial.println(data);
      char output[8] = {};
      data.toCharArray(output,8); // why do we need output?? We don't seem to use it..
      ble.print(data);
  }
